...
CLASS zcl_zork_00_io_html DEFINITION
  ...
  PUBLIC SECTION.
    ...
    DATA: mo_html_viewer TYPE REF TO cl_gui_html_viewer READ-ONLY.
...
CLASS zcl_zork_00_io_html IMPLEMENTATION.
  ...
  METHOD zif_zork_00_io~print_char.
    " Convert ZSCII code to character and append (with HTML escaping)
    ...
    ELSEIF iv_zscii >= 32 AND iv_zscii <= 126.
      ...
      CASE iv_zscii.
        WHEN 32. lv_html = '&nbsp;'.   " Space
        WHEN 34. lv_html = '&quot;'.   " "
        WHEN 38. lv_html = '&amp;'.    " &
        WHEN 60. lv_html = '&lt;'.     " <
        WHEN 62. lv_html = '&gt;'.     " >
        WHEN OTHERS.
          " Use cl_abap_conv_in_ce for other characters
          lv_char = cl_abap_conv_in_ce=>uccpi( iv_zscii ).
          lv_html = lv_char.
      ENDCASE.
      mv_output = mv_output && lv_html.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD zif_zork_00_io~read_char.
    ...
    IF mv_input_queue IS NOT INITIAL.
      ...
      " Return ASCII code (simplified)
      CASE lv_char.
        WHEN ' '. rv_zscii = 32.
        WHEN OTHERS.
          " For letters, use ordinal
          rv_zscii = 0.
      ENDCASE.
    ELSE.
      mv_waiting_input = abap_true.
